  ...
  kontener.append(
    ...
    el('div', { class: 'karta-box' },
      ...
      el('div', { class: 'szczegoly' },
        ...
        poz('Opiekun konta / KAM', k.opiekun || '—'),
        poz('Następny przegląd konta (AM)', k.data_nastepnego_przegladu ? dataPl(k.data_nastepnego_przegladu) : '— brak planu opieki —'),
...
function formularzKlienta(sl, k, poZapisie) {
  const form = el('div', { class: 'form-siatka' },
    ...
    pole({ name: 'opiekun', label: 'Opiekun konta / KAM', wartosc: k?.opiekun, pomoc: 'Może być inny niż handlowiec konkretnego tematu' }),
    pole({ name: 'data_nastepnego_przegladu', label: 'Następny przegląd konta (plan opieki AM)', typ: 'date', wartosc: k?.data_nastepnego_przegladu, pomoc: 'Rytm opieki nad kontem powracającym — zasila coverage w metrykach' }),
