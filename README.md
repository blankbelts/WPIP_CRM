...
| **Prognoza sprzedaży** | Lejek konwersji New Business (leady → kwalifikacja → interesujące → Komitet → pipeline → wygrane z % konwersji), prognoza przychodu kwartalna (pipeline ważony prawdopodobieństwem, wykres + tabela) oraz potencjał New Business z lejka (interesujące × bid-rate × win-rate × śr. kontrakt, wg baseline 2025). Endpoint `/prognoza` |
| **Kwalifikuj wstępnie (masowo)** | Auto-triage leadów „Lead surowy" na podstawie danych z importu — heurystyka odpowiada na pytania kwalifikacji z profilu (typologia, wartość, profil inwestora, dopasowanie) i przypisuje proces researchu; interesujące → kwalifikacja, twarda dyskwalifikacja → odpuszczone. Werdykty korygowalne ręcznie. Endpoint `/leady/kwalifikuj-wstepnie`, logika `kwalifikacja.js` (`autoOdpowiedzi`, `autoProces`) |
